...
<%
// PlatformData
PlatformData out_pData = new PlatformData(); // 깡통을 하나 만들고
String sDept = (request.getParameter("sDept") == null) ? "" : request.getParameter("sDept");
	
int    nErrorCode  = 0;
String strErrorMsg = "START";

HttpPlatformRequest pReq = new HttpPlatformRequest(request); // 클라이언트에서 서버로 넘겨준 리퀘스트 정보를 꺼내는 것 
pReq.receiveData();
PlatformData in_pData = pReq.getData(); // 서버에서 보내준 데이터를 깡통에 담고 

VariableList in_varList = in_pData.getVariableList(); // 파라미터값이 들어있다... 
